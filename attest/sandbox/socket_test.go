...
// TestAReadDeadlineOnAStreamOverTheSocketExpires is the same deadline at the
// other implementation (ticket 23). A sandbox in another process holds one end
// of a socketpair rather than a QUIC stream, so the deadline it sets is the
// kernel's and not quic-go's; what the contract promises is that the two are
// indistinguishable from inside the sandbox.
//
// The client is dialed from this process, which is the only thing the child in
// the test above does differently: what is asked here is whether a descriptor
// that arrived over SCM_RIGHTS takes a deadline, and that question does not
// need a second process to answer.
func TestAReadDeadlineOnAStreamOverTheSocketExpires(t *testing.T) {
	fake := newFakeNetwork()
	host, err := sandbox.Listen(filepath.Join(t.TempDir(), "sandbox.sock"), fake, nil)
	if err != nil {
		t.Fatalf("listening: %v", err)
	}
	defer host.Close()
	...
	defer stream.Close()
	// The peer's end, held open and silent: closing it would end the stream and
	// the sandbox would read end-of-file instead of waiting.
	far := fake.nextOpened(t)
	defer far.Close()
