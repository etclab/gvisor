...
process boundary in the middle (`host.go:50`). A `context.Context` is on each method because
...
(`sandbox.Listen`, `host.go:86`, mode 0600, the directory created if missing, a socket left by a
