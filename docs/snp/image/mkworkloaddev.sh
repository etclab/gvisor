#!/bin/bash
# Build the workload device: the read-only block device the measured initrd
# mounts at /workload, OUTSIDE the launch measurement. It carries one OCI bundle
# and nothing else. Changing what is on it does not change the measurement, and
# what the measurement does cover is that runsc, the flag set /sbin/init runs it
# with and this mount's options are in the image (ticket 24).
#
#   mkworkloaddev.sh SRCDIR OUT.img
#
...
# exec-permitted is the shape that works, and it satisfies the guest's rule
# without an exception: the rule constrains writable mounts, and this one is not
# writable.
#
...
# ext4, no journal (read-only), populated from SRC without needing root on the
# host. The label is truncated to ext4's 16 bytes — 'attested-workloa' — and
# nothing reads it: the disk is found inside the guest by its virtio-blk serial,
# which is the whole name (init.initrd's find_disk).
mke2fs -q -t ext4 -O ^has_journal -L attested-workload -m 0 -E root_owner=0:0 -d "$SRC" "$OUT"
...
echo "workload device sha256: $(sha256sum "$OUT" | cut -d' ' -f1)  (not measured)"
