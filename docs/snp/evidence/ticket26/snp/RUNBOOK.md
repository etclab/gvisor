...
| **the liveness refusal sentence** — **landed** | the contract agent: `attest/refusal.go:147`, the eleventh reason, wired at `attest/tunneld/push.go:256` | `grep -n 'ReasonPolicyNotLive' attest/refusal.go` | the harness greps for one variable, `LIVENESS_REASON`, defaulting to *"the policy pushed to the peer is no longer live"* — which is the string that landed, checked rather than guessed. The whole console line is `tunneld: REFUSED verification refused: the policy pushed to the peer is no longer live: a peer at <addr> pushed a policy this sandbox no longer enforces: <why>` (`Refusal.LogString`, `refusalLogger`). If it is ever reworded, run with `LIVENESS_REASON='<the real one>'` and then fix the default |
| **the `exec_refused` reason string** — **landed** | the sentry agent: `pkg/sentry/seccheck/metadata.go:349` (`sentry/exec_refused`), `points/sentry.proto:223` (`ExecRefused`, `reason` is `"not-in-x"`) | `grep -rn 'exec_refused' pkg/sentry/seccheck/` | the run does **not** assert on it — there is no seccheck receiver inside a measured guest. It asserts on the workload's own `EXEC REFUSED /bin/probe` line, which is the errno the shell reported (`sh: /bin/probe: Permission denied`, checked against this exact busybox). Record the reason string in `notes.md` from the loopback spikes instead |
