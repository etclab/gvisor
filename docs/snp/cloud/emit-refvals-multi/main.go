...
// emit-refvals-multi is docs/snp/image/emit-refvals with more than one value.
//
// A mixed run has two peers that cannot run the same image, so each side's
// set has to admit the other's measurement, and a set is a list precisely so
// that it can (attest/README.md). Each -value is MEASUREMENT:b,t,s,m:POLICY —
// the hex launch measurement, its own TCB floor, its own guest policy —
// because the two platforms are different product lines at different TCBs and
// one floor would be wrong for one of them.
//
//	emit-refvals-multi -key author.key -out DIR \
//	    -value 0ecf…:9,0,23,72:0x30000 -value 2d24…:4,0,29,222:0x30000
//
// Signing and loading back are the attest package's own, as in emit-refvals.
package main
...
func main() {
	var specs values
	flag.Var(&specs, "value", "MEASUREMENT_HEX:bootloader,tee,snp,microcode:POLICY (repeatable)")
	...
	if err := run(specs, *keyPath, *out); err != nil {
		fmt.Fprintln(os.Stderr, "emit-refvals-multi:", err)
		os.Exit(1)
	}
}
...
func run(specs []string, keyPath, out string) error {
	...
	for _, spec := range specs {
		...
		if len(parts) != 3 {
			return fmt.Errorf("-value %q: want MEASUREMENT:b,t,s,m:POLICY", spec)
		}
		m, err := hex.DecodeString(parts[0])
		if err != nil || len(m) != 48 {
			return fmt.Errorf("-value %q: measurement is not 48 bytes of hex", spec)
		}
		floor, err := parseTCB(parts[1])
		if err != nil {
			return fmt.Errorf("-value %q: %v", spec, err)
		}
	...
	if len(loaded.Values) != len(set.Values) {
		return fmt.Errorf("the emitted set loaded back with %d values, want %d", len(loaded.Values), len(set.Values))
	}
