...
//
// Ticket 26's copy adds two points to ticket 25's: sentry/exec_refused (message
// type 40), which is an execve the policy's x did not name, and sentry/execve
// (type 3), which the exec sink turns on wherever it is installed and which is
// how a run's list of (path, sha256) identities is read off.
package main
...
func print(b []byte) {
	...
	case msgSentryExecRefused:
		fmt.Println(execRefused(payload, dropped))
	case msgSentryExecve:
		fmt.Println(execve(payload, dropped))
...
// execRefused formats a sentry/exec_refused point. The field numbers are
// ExecRefused's in pkg/sentry/seccheck/points/sentry.proto: context_data 1,
// path 2, sha256 3, reason 4.
func execRefused(b []byte, dropped uint32) string {
	var m message
	m.parse(b)
	var ctx message
	ctx.parse(m.bytes(1))
	var sb strings.Builder
	sb.WriteString("exec_refused")
	fmt.Fprintf(&sb, " path=%s", quoteEmpty(m.str(2)))
	fmt.Fprintf(&sb, " sha256=%s", quoteEmpty(m.str(3)))
	fmt.Fprintf(&sb, " reason=%s", quoteEmpty(m.str(4)))
	sb.WriteString(contextSuffix(&ctx, dropped))
	return sb.String()
}

// execve formats a sentry/execve point, which the exec sink turns on wherever
// it is installed: X is decided on the identity this point already carries, so
// a trace of one run lists every (path, sha256) the sandbox executed. The
// field numbers are ExecveInfo's: binary_path 2, binary_sha256 8.
func execve(b []byte, dropped uint32) string {
	var m message
	m.parse(b)
	var ctx message
	ctx.parse(m.bytes(1))
	var sb strings.Builder
	sb.WriteString("execve")
	fmt.Fprintf(&sb, " path=%s", quoteEmpty(m.str(2)))
	if sum := m.bytes(8); len(sum) > 0 {
		fmt.Fprintf(&sb, " sha256=%x", sum)
	} else {
...
// contextSuffix is the three context fields every point here carries, printed
// the same way for each of them.
func contextSuffix(ctx *message, dropped uint32) string {
	var sb strings.Builder
	if t := ctx.varint(1); t != 0 {
		fmt.Fprintf(&sb, " time=%s", time.Unix(0, int64(t)).UTC().Format(time.RFC3339Nano))
	}
