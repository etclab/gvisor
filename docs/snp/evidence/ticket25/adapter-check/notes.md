...
`ticket-25-the-adapter-intercept-and-handoff`. No sudo, no hardware.
    ...
    runsc sha256           d9abdc6a3d82af1ac280bb30e1a101d209d91c59f12a2e7456f8758e0b330360
...
| Node | `ALLOWED-NAME status=200 tls=TLSv1.3 bytes=179553 ms=418` |
...
and the sentry's own account of the same two connections:
    ...
    tunnel attach: www.rfc-editor.org:443 -> peer "b": ok in 28.859496ms, host fd 48, local 100.64.0.1:40001
...
refused it.** The Go workload's six lines, and the receiver's:
    ...
    egress_refused protocol=tcp address=100.64.1.1 port=80 name=www.rfc-editor.org reason=wrong-port  container_id=t25-go-412438 thread_id=1
    egress_refused protocol=dns name=example.com reason=unknown-name
    egress_refused protocol=dns name=example.com reason=unknown-name
    egress_refused protocol=tcp address=8.8.8.8 port=443 reason=not-in-table     container_id=t25-go-412438 thread_id=1
    egress_refused protocol=udp address=8.8.8.8 port=53  reason=not-in-table     container_id=t25-go-412438 thread_id=1
    egress_refused protocol=udp address=8.8.8.8 port=53  reason=not-in-table     container_id=t25-go-412438 thread_id=1
...
**3. A destination the far exit refuses is `ECONNREFUSED` and is not recorded.**
    ...
    tunnel attach: refused.example:443 -> peer "b": refused by the far exit in 5.1384ms
...
and it does not hang.** Both of the two ways it can happen:
    ...
    wrong-peer: tunnel attach: www.rfc-editor.org:443 -> peer "zz": unavailable in 2.886182ms
                (asking tunneld for a stream to peer "zz": no peer named "zz")
    no-helper:  tunnel attach: www.rfc-editor.org:443 -> peer "b": unavailable in 425.324µs
