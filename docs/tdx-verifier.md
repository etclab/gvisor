...
- **The policy hash in user data** as this verifier saw it when it was written. Ticket 18 has since
  ...
  the recorded quotes are still judged by the same code, and a peer presenting a policy is tested
  against the fake TDX platform, because a recording's report data is fixed and cannot be bound to
  a policy chosen afterwards.
...
# a recorded quote against the recorded collateral and a signed version-4 set
# (the recorded reference-values.v20260826.json is version 2 and no longer loads;
#  re-emit it — docs/snp/cloud/tdx/emit-tdx-refvals. A version 4 set carries no
#  egress section: since ticket 19 that lives in the sandbox's own policy.)
