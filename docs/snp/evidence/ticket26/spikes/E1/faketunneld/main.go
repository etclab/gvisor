// A throwaway stand-in for tunneld, for ticket 26's spike E1.
//
...
//
//	apply <path>   push the bytes of that file and print the answer
...
func applyOne(h *sandbox.Host, path string) {
	...
	log.Printf("APPLY %s ACK elapsed=%v bytes=%d", path, elapsed, len(policy))
}
